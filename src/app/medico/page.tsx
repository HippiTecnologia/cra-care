    ...
    <main className="min-h-screen bg-[#f8f5f2] text-[#34292d]">
      <div className="min-h-screen lg:grid lg:grid-cols-[285px_minmax(0,1fr)]">
        ...
        <section className="min-w-0 px-5 py-8 sm:px-8 lg:px-10">
          ...
          <section className="mt-8 rounded-3xl border border-[#efe6e1] bg-white p-6 shadow-sm"><h2 className="text-xl font-bold text-[#433438]">Evolução pela avaliação da imunoterapia</h2><p className="mt-1 text-sm text-[#817578]">Dados reais do histórico de cada paciente.</p><div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-3">{evolution.filter(item=>item.latest).map(({patient,latest})=><Link key={patient.id} href={`/medico/paciente/${patient.id}`} className="rounded-2xl bg-[#fbf7f5] p-4 hover:bg-[#fff0f3]"><p className="font-bold">{patient.name}</p><p className="mt-2 text-sm">Perspectiva de melhorar: <strong>{latest?.nuisanceScore ?? "—"}/10</strong></p><p className="mt-2 text-xs text-[#817578]">{latest?.assessmentType === "inicial" ? "Avaliação inicial" : "Acompanhamento"} · Data da última avaliação: {formatDate(latest?.createdAt ?? "")}</p></Link>)}{!evolution.some(item=>item.latest)&&<p className="text-sm text-[#817578]">As avaliações preenchidas pelos pacientes aparecerão aqui.</p>}</div></section>
          </>}
