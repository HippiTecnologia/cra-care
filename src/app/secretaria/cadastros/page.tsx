...
function mergePatients() {
  ...
  const completedSaved = saved.map((patient) => {
    const demoPatient = demoMedicalPatients.find((item) => item.id === patient.id);
    return demoPatient && !patient.email ? { ...patient, email: demoPatient.email } : patient;
  });
  return [...completedSaved, ...demoMedicalPatients.filter((item) => !savedIds.has(item.id))];
}
