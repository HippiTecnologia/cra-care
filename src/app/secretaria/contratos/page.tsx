...
export default function ContractsPage() {
  ...
  function openContract(patient: DemoPatientRecord) {
    ...
    popup.document.write(`<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><title>Termo de adesão · ${escapeHtml(patient.name)}</title><style>@page{size:A4 portrait;margin:14mm}*{box-sizing:border-box}body{font:13px Arial,sans-serif;max-width:820px;margin:26px auto;padding:0 22px;color:#3f3236;line-height:1.58}header{background:#a3113a;color:white;padding:24px;border-radius:16px}header img{width:118px;height:auto}h1{font-size:23px;margin:15px 0 4px}header p{margin:0;color:#ffe7ed}section{margin-top:16px;padding:16px;background:#faf7f5;border:1px solid #eee3de;border-radius:12px;break-inside:avoid}h2{margin:0 0 8px;color:#86203b;font-size:15px}section p{margin:0;white-space:normal}.status{margin-top:16px;padding:13px;border-radius:12px;background:${portal.signedAt ? "#edf8f3" : "#fff4e4"};color:${portal.signedAt ? "#187157" : "#966419"};font-weight:bold}.actions{display:flex;gap:12px;margin:20px 0}.actions button{border:0;border-radius:10px;background:#a3113a;color:white;padding:12px 18px;font-weight:bold;cursor:pointer}@media print{body{margin:0 auto;padding:0;max-width:none}.actions{display:none}header{print-color-adjust:exact;-webkit-print-color-adjust:exact}}</style></head><body><header><img src="${window.location.origin}/logo-cra-branca.png" alt="CRA Care"><h1>Termo de adesão — Imunoterapia Alérgeno Específica (IAE)</h1><p>CRA Care · Centro de Rinite e Alergia</p></header><div class="status">Status: ${portal.signedAt ? `Assinado eletronicamente em ${escapeHtml(formatDate(portal.signedAt, true))}` : "Pendente de assinatura"}</div>${sections}<div class="actions"><button onclick="window.print()">Imprimir / Salvar em PDF</button></div></body></html>`);
