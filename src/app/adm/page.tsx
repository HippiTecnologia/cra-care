...
function buildReportRows(type: ReportType, sales: AdminSaleSnapshot[], installments: InstallmentView[], patients: DemoPatientRecord[], invoices: DemoInvoice[], costs: AdminFixedCost[]): ReportRow[] {
  ...
  if (type === "Comissões") return installments.filter((item) => item.commissionAt).map((item) => ({ Médico: item.sale.doctor, Paciente: item.sale.patientName, Parcela: `${item.number}/${item.sale.installments}`, Frascos: item.bottleCount, "Comissão por frasco": formatMoney(item.commissionPerBottle), Comissão: formatMoney(item.commissionValue), "Data do pagamento": formatDate(item.payment?.paidAt), "Data para comissão": formatDate(item.commissionAt), "Status da comissão": item.commissionRecord ? `Paga em ${formatDate(item.commissionRecord.paidAt)}` : "Disponível para pagamento", "Nota fiscal": item.invoice?.fileName ?? "Não vinculada" }));
