...
function InstallmentTable({ installments, showCommission, openInvoice, onMarkCommission, onReverseCommission }: { installments: InstallmentView[]; showCommission?: boolean; openInvoice: (invoice?: DemoInvoice) => void; onMarkCommission?: (installment: InstallmentView) => void; onReverseCommission?: (installment: InstallmentView) => void }) {
  const today = new Date().toISOString().slice(0, 10);
  return <div className="mt-4 max-h-[680px] overflow-auto"><table className="w-full min-w-[1400px] text-left text-sm"><thead className="sticky top-0 bg-white"><tr><Th>Paciente</Th><Th>Médico</Th><Th>Tratamento</Th><Th>Venda</Th><Th>Parcela</Th><Th>Valor</Th><Th>Vencimento</Th><Th>Pagamento</Th><Th>Forma</Th>{showCommission && <><Th>Frascos</Th><Th>R$/frasco</Th><Th>Honorário</Th><Th>Data p/ honorário</Th><Th>Situação do honorário</Th>{onMarkCommission && <Th>Ação</Th>}</>}<Th>Status</Th><Th>Nota fiscal</Th></tr></thead><tbody>{installments.map((item) => {
    const honorariumDue = Boolean(item.commissionAt && item.commissionAt <= today);
    const honorariumLabel = item.commissionRecord ? `Pago ${formatDate(item.commissionRecord.paidAt)}` : honorariumDue ? "Disponível" : "Agendado";
    const honorariumColor = item.commissionRecord ? "bg-[#edf8f3] text-[#187157]" : honorariumDue ? "bg-[#fff4e4] text-[#966419]" : "bg-[#eef3ff] text-[#3c5da0]";
    return <tr key={item.id}><Td strong>{item.sale.patientName}</Td><Td>{item.sale.doctor}</Td><Td>{item.sale.treatment}</Td><Td>{formatMoney(item.sale.contractedValue)}</Td><Td>{item.number}/{item.sale.installments}</Td><Td strong>{formatMoney(item.scheduledValue)}</Td><Td>{formatDate(item.dueAt)}</Td><Td>{formatDate(item.payment?.paidAt)}</Td><Td>{item.payment?.method ?? item.sale.paymentMethod}</Td>{showCommission && <><Td>{item.commissionAt ? item.bottleCount : "—"}</Td><Td>{item.commissionAt ? formatMoney(item.commissionPerBottle) : "—"}</Td><Td>{item.commissionAt ? formatMoney(item.commissionValue) : "—"}</Td><Td>{formatDate(item.commissionAt)}</Td><Td>{item.commissionAt ? <span className={`rounded-full px-2 py-1 text-[10px] font-bold ${honorariumColor}`}>{honorariumLabel}</span> : "—"}</Td>{onMarkCommission && <Td>{item.commissionAt ? item.commissionRecord ? <button type="button" onClick={() => onReverseCommission?.(item)} className="font-bold text-[#a3113a]">Estornar</button> : honorariumDue ? <button type="button" onClick={() => onMarkCommission(item)} className="rounded-lg bg-[#a3113a] px-3 py-2 text-xs font-bold text-white">Marcar como pago</button> : <span className="text-xs text-[#817578]">Aguardar vencimento</span> : "—"}</Td>}</>}<Td><span className={`rounded-full px-2 py-1 text-[10px] font-bold ${item.status === "Recebida" ? "bg-[#edf8f3] text-[#187157]" : item.status === "Vencida" ? "bg-[#fff0ef] text-[#b54843]" : "bg-[#fff4e4] text-[#966419]"}`}>{item.status}</span></Td><Td>{item.invoice ? <div className="flex gap-2"><button type="button" onClick={() => openInvoice(item.invoice)} className="font-bold text-[#a3113a]">Abrir</button><a href={item.invoice.fileData} download={item.invoice.fileName} className="font-bold text-[#187157]">Baixar</a></div> : "Não vinculada"}</Td></tr>;
  })}</tbody></table>{installments.length === 0 && <p className="py-10 text-center text-sm text-[#817578]">Nenhuma parcela encontrada.</p>}</div>;
}
