...
function createInstallments(
  ...
  return sales.flatMap((sale) => {
    ...
    return Array.from({ length: count }, (_, index) => {
      ...
      const isCommissionPayment = Boolean(payment && firstPayment && payment.id === firstPayment.id);
      return {
        id: installmentId,
        sale,
        number: index + 1,
        ...
        payment,
        receivedValue,
        commissionValue: isCommissionPayment ? bottleCount * commissionPerBottle : 0,
        bottleCount,
        commissionPerBottle,
        commissionAt: isCommissionPayment ? commissionDate(payment) : undefined,
        commissionRecord,
        status,
    ...
    <main className="min-h-screen bg-[#f6f3f0] text-[#34292d]">
      <div className="min-h-screen">
        ...
        <div className="min-w-0 p-4 sm:p-7 lg:ml-[280px] xl:p-9">
          ...
          {section === "medicos" && <div className="mt-7 space-y-5"><Panel title="Cadastro e desempenho por médico" subtitle="A comissão é fixa: R$ 64,00 por frasco, calculada automaticamente pelo método contratado."><div className="mt-5 grid gap-4 xl:grid-cols-2">{doctorPerformance.map((item) => <article key={item.doctor.id} className="rounded-2xl border border-[#eee5e0] p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="font-bold">{item.doctor.name}</h3><p className="mt-1 text-xs text-[#817578]">{item.doctor.crm ? `CRM ${item.doctor.crm}` : "CRM a cadastrar"} · {item.patients} paciente(s)</p></div><span className="rounded-xl bg-[#edf8f3] px-3 py-2 text-xs font-bold text-[#187157]">R$ 64,00 por frasco</span></div><div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-3"><Mini label="Tratamentos" value={String(item.treatments)} /><Mini label="Total vendido" value={formatMoney(item.sold)} /><Mini label="Recebido" value={formatMoney(item.received)} /><Mini label="Pendente" value={formatMoney(item.pending)} /><Mini label="Comissões" value={formatMoney(item.commissions)} /><Mini label="Regra atual" value="R$ 64,00 / frasco" /></div><div className="mt-4"><p className="text-xs font-bold text-[#817578]">Comparativo por mês</p><div className="mt-2 flex gap-2 overflow-x-auto">{monthly.slice(-6).map((month) => { const amount = installments.filter((installment) => installment.sale.doctor === item.doctor.name && installment.commissionAt && monthKey(installment.commissionAt) === month.month).reduce((sum, installment) => sum + installment.commissionValue, 0); return <div key={month.month} className="min-w-24 rounded-xl bg-[#fbf7f5] p-3"><p className="text-[10px] capitalize text-[#817578]">{monthLabel(month.month)}</p><p className="mt-1 text-xs font-bold">{formatMoney(amount)}</p></div>; })}</div></div></article>)}</div></Panel></div>}
