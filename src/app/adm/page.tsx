...
type ReportType = "Vendas" | "Recebimentos" | "Parcelas" | "Notas fiscais" | "Honorários" | "Custos" | "Vendas por médico" | "Conversões" | "Desistências";
...
const sections: Array<{ id: Section; icon: string; label: string }> = [
  ...
  { id: "comissoes", icon: "%", label: "Honorários" },
...
const reportTypes: ReportType[] = ["Vendas", "Recebimentos", "Parcelas", "Notas fiscais", "Honorários", "Custos", "Vendas por médico", "Conversões", "Desistências"];
...
function displayHonorarios(value: string) {
  return value
    .replaceAll("Comissões", "Honorários")
    .replaceAll("comissões", "honorários")
    .replaceAll("Comissão", "Honorário")
    .replaceAll("comissão", "honorário");
}
...
export default function AdminPage() {
  ...
  const filteredInstallments = baseInstallments.filter((installment) => {
    ...
    if (reportType === "Honorários") return inReportPeriod(installment.commissionAt);
  ...
  const commissionMonths = useMemo(
    () => Array.from(new Set(installments.flatMap((item) => item.commissionAt ? [monthKey(item.commissionAt)] : []))).sort(),
    [installments],
  );
  const selectedCommissionMonth = commissionMonths.includes(commissionMonth)
    ? commissionMonth
    : (commissionMonths.find((month) => month >= monthKey(currentDate)) ?? commissionMonths.at(-1) ?? "");
  const commissionInstallments = useMemo(() => installments.filter((item) => {
    if (!item.commissionAt || item.status === "Cancelada") return false;
  ...
  function editCost(cost: AdminFixedCost) {
    setCostDraft({ ...cost });
    setMessage(`Editando ${cost.description}. Altere os campos e salve.`);
  ...
  async function updateDoctorBottleCommission(doctor: AdminDoctor) {
    ...
    if (!Number.isFinite(value) || value < 0) {
      setMessage("Informe um valor válido de honorário por frasco.");
      return;
    }
    if (!context) return setMessage("Sessão administrativa não identificada.");
    try {
      ...
      await reloadAdmin();
      setMessage("Honorário por frasco atualizado para novas vendas. Vendas anteriores preservam o valor contratado.");
    } catch (cause) {
      setMessage(cause instanceof Error ? cause.message : "Não foi possível atualizar o honorário.");
    }
  }
  ...
  async function markCommissionAsPaid(installment: InstallmentView) {
    ...
    try {
      ...
      await reloadAdmin();
      setMessage(`Honorário de ${formatMoney(installment.commissionValue)} marcado como pago para ${installment.sale.doctor}.`);
    } catch (cause) {
      setMessage(cause instanceof Error ? cause.message : "Não foi possível baixar o honorário.");
    }
  }

  async function reverseCommissionPayment(installment: InstallmentView) {
    if (!installment.commissionRecord) return;
    if (!window.confirm(`Estornar o pagamento do honorário de ${formatMoney(installment.commissionValue)} para ${installment.sale.doctor}?`)) return;
    if (!context) return setMessage("Sessão administrativa não identificada.");
    try {
      await reverseAdminCommissionPayment(context, installment.commissionRecord);
      await reloadAdmin();
      setMessage("Pagamento de honorário estornado. A parcela volta para a lista de disponíveis.");
    } catch (cause) {
      setMessage(cause instanceof Error ? cause.message : "Não foi possível estornar o honorário.");
    }
  }
    ...
    return (
      <main className="flex min-h-screen items-center justify-center bg-[#f6f3f0] px-5 text-[#34292d]">
        <section className="w-full max-w-xl rounded-3xl border border-[#eadfd9] bg-white p-7 shadow-sm">
          ...
          <div className="mt-5 rounded-2xl border border-[#f1cdd6] bg-[#fff4f6] p-4 text-sm font-semibold text-[#a3113a]">{displayHonorarios(loadError || "Não foi possível validar o acesso administrativo.")}</div>
  ...
  return (
    <main className="min-h-screen bg-[#f6f3f0] text-[#34292d]">
      <div className="min-h-screen">
        <aside className="bg-gradient-to-b from-[#701027] via-[#8f1033] to-[#520817] px-6 py-7 text-white lg:fixed lg:inset-y-0 lg:left-0 lg:z-40 lg:w-[280px] lg:overflow-y-auto">
        ...
        <div className="min-w-0 p-4 sm:p-7 lg:ml-[280px] xl:p-9">
          ...
          {section === "medicos" && <div className="mt-7 space-y-5"><Panel title="Cadastro e desempenho por médico" subtitle="O honorário por frasco é editável pelo ADM e fica registrado no momento de cada nova venda."><div className="mt-5 grid gap-4 xl:grid-cols-2">{doctorPerformance.map((item) => <article key={item.doctor.id} className="rounded-2xl border border-[#eee5e0] p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="font-bold">{item.doctor.name}</h3><p className="mt-1 text-xs text-[#817578]">{item.doctor.crm ? `CRM ${item.doctor.crm}` : "CRM a cadastrar"} · {item.patients} paciente(s)</p></div><div className="flex items-end gap-2"><Input compact label="Honorário (R$/frasco)" type="number" value={String(doctorBottleCommissions[item.doctor.id] ?? item.doctor.commissionPerBottle ?? COMMISSION_PER_BOTTLE)} onChange={(value) => setDoctorBottleCommissions((current) => ({ ...current, [item.doctor.id]: Number(value) }))} /><button type="button" onClick={() => updateDoctorBottleCommission(item.doctor)} className="h-10 rounded-xl bg-[#a3113a] px-3 text-xs font-bold text-white">Salvar</button></div></div><div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-3"><Mini label="Tratamentos" value={String(item.treatments)} /><Mini label="Total vendido" value={formatMoney(item.sold)} /><Mini label="Recebido" value={formatMoney(item.received)} /><Mini label="Pendente" value={formatMoney(item.pending)} /><Mini label="Honorários previstos" value={formatMoney(item.commissions)} /><Mini label="Regra atual" value={`${formatMoney(item.doctor.commissionPerBottle ?? COMMISSION_PER_BOTTLE)} / frasco`} /></div><div className="mt-4"><p className="text-xs font-bold text-[#817578]">Honorários por competência</p><div className="mt-2 flex gap-2 overflow-x-auto">{monthly.slice(-6).map((month) => { const amount = installments.filter((installment) => installment.sale.doctor === item.doctor.name && installment.commissionAt && monthKey(installment.commissionAt) === month.month).reduce((sum, installment) => sum + installment.commissionValue, 0); return <div key={month.month} className="min-w-24 rounded-xl bg-[#fbf7f5] p-3"><p className="text-[10px] capitalize text-[#817578]">{monthLabel(month.month)}</p><p className="mt-1 text-xs font-bold">{formatMoney(amount)}</p></div>; })}</div></div></article>)}</div></Panel></div>}

          {section === "comissoes" && <div className="mt-7 space-y-5">
            <Panel title="Fechamento de honorários" subtitle="O honorário acompanha cada parcela recebida: PIX e débito no dia; cartão 30 dias após o recebimento.">
            ...
            <div className="grid gap-4 sm:grid-cols-3"><Kpi label="Honorários disponíveis" value={formatMoney(commissionAvailable)} detail="Ainda não baixados no período" tone="wine" /><Kpi label="Honorários já pagos" value={formatMoney(commissionPaid)} detail="Baixados no período filtrado" tone="green" /><Kpi label="Parcelas no fechamento" value={String(commissionInstallments.length)} detail="Vencimentos e agendamentos" tone="blue" /></div>
          ...
          {section === "auditoria" && <div className="mt-7 space-y-5"><div className="grid gap-4 sm:grid-cols-3"><Kpi label="Alterações registradas" value={String(audit.length)} detail="Valores, custos e honorários" tone="wine" /><Kpi label="Vendas preservadas" value={String(sales.length)} detail="Snapshots financeiros" tone="green" /><Kpi label="Modalidades versionadas" value={String(methods.length)} detail="Histórico independente" tone="blue" /></div><Panel title="Histórico de configurações" subtitle="Registro de quem alterou, quando e o que foi modificado"><div className="mt-5 space-y-3">{audit.map((entry) => <article key={entry.id} className="flex flex-col gap-2 rounded-2xl bg-[#fbf7f5] p-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm font-bold">{displayHonorarios(entry.action)} · {displayHonorarios(entry.entity)}</p><p className="mt-1 text-xs text-[#66595d]">{displayHonorarios(entry.summary)}</p></div><p className="text-xs text-[#817578]">{entry.createdBy}<br />{formatDate(entry.createdAt, true)}</p></article>)}{audit.length === 0 && <p className="py-8 text-center text-sm text-[#817578]">As próximas alterações aparecerão aqui.</p>}</div></Panel><Panel title="Regra de preservação histórica" subtitle="Aplicada a todas as vendas"><div className="mt-4 rounded-2xl border border-[#d7e9df] bg-[#edf8f3] p-5 text-sm leading-7 text-[#187157]">Cada venda guarda: paciente, tratamento, versão do método, valor contratado, desconto, condição escolhida, forma de pagamento, parcelas, quantidade de frascos e honorário fixo de R$ 68,00 por frasco. Mudanças futuras feitas pelo ADM afetam somente novas vendas.</div></Panel></div>}
        </div>
      </div>
    </main>
  );
...
function buildReportRows(type: ReportType, sales: AdminSaleSnapshot[], installments: InstallmentView[], patients: DemoPatientRecord[], invoices: DemoInvoice[], costs: AdminFixedCost[]): ReportRow[] {
  ...
  if (type === "Honorários") return installments.filter((item) => item.commissionAt).map((item) => ({ Médico: item.sale.doctor, Paciente: item.sale.patientName, Parcela: `${item.number}/${item.sale.installments}`, Frascos: item.bottleCount, "Honorário por frasco": formatMoney(item.commissionPerBottle), Honorário: formatMoney(item.commissionValue), "Data do pagamento": formatDate(item.payment?.paidAt), "Data para honorário": formatDate(item.commissionAt), "Status do honorário": item.commissionRecord ? `Pago em ${formatDate(item.commissionRecord.paidAt)}` : "Disponível para pagamento", "Nota fiscal": item.invoice?.fileName ?? "Não vinculada" }));
  ...
  if (type === "Vendas por médico") return Array.from(new Set(sales.map((sale) => sale.doctor))).map((doctor) => { const doctorSales = sales.filter((sale) => sale.doctor === doctor); const doctorInstallments = installments.filter((item) => item.sale.doctor === doctor); return { Médico: doctor, Tratamentos: doctorSales.length, Pacientes: new Set(doctorSales.map((sale) => sale.patientId)).size, Vendido: formatMoney(doctorSales.reduce((sum, sale) => sum + sale.contractedValue, 0)), Recebido: formatMoney(doctorInstallments.reduce((sum, item) => sum + item.receivedValue, 0)), Pendente: formatMoney(doctorInstallments.filter((item) => item.status !== "Recebida" && item.status !== "Cancelada").reduce((sum, item) => sum + item.scheduledValue, 0)), Honorários: formatMoney(doctorInstallments.reduce((sum, item) => sum + item.commissionValue, 0)) }; });
...
function InstallmentTable({ installments, showCommission, openInvoice, onMarkCommission, onReverseCommission }: { installments: InstallmentView[]; showCommission?: boolean; openInvoice: (invoice?: DemoInvoice) => void; onMarkCommission?: (installment: InstallmentView) => void; onReverseCommission?: (installment: InstallmentView) => void }) {
  const today = new Date().toISOString().slice(0, 10);
  const [openSales, setOpenSales] = useState<string[]>([]);
  const isOpen = (saleId: string) => openSales.includes(saleId);
  const toggle = (saleId: string) => setOpenSales((current) => current.includes(saleId) ? current.filter((id) => id !== saleId) : [...current, saleId]);
  const statusPill = (item: InstallmentView) => <span className={`rounded-full px-2 py-1 text-[10px] font-bold ${item.status === "Recebida" ? "bg-[#edf8f3] text-[#187157]" : item.status === "Vencida" ? "bg-[#fff0ef] text-[#b54843]" : "bg-[#fff4e4] text-[#966419]"}`}>{item.status}</span>;
  const commissionAction = (item: InstallmentView) => {
    const due = Boolean(item.commissionAt && item.commissionAt <= today);
    if (item.commissionRecord) return <button type="button" onClick={() => onReverseCommission?.(item)} className="font-bold text-[#a3113a]">Estornar</button>;
    if (due) return <button type="button" onClick={() => onMarkCommission?.(item)} className="rounded-lg bg-[#a3113a] px-3 py-2 text-xs font-bold text-white">Marcar como pago</button>;
    return <span className="text-xs text-[#817578]">Aguardar vencimento</span>;
  };

  if (showCommission) {
    const groups = Array.from(installments.reduce((map, item) => {
      map.set(item.sale.id, [...(map.get(item.sale.id) ?? []), item]);
      return map;
    }, new Map<string, InstallmentView[]>()).values());
    return <div className="mt-4 max-h-[680px] overflow-auto"><table className="w-full min-w-[1850px] text-left text-sm"><thead className="sticky top-0 bg-white"><tr><Th>Paciente</Th><Th>Médico</Th><Th>Tratamento</Th><Th>Venda</Th><Th>Parcelas</Th><Th>Valor</Th><Th>Vencimento</Th><Th>Pagamento</Th><Th>Forma</Th><Th>Frascos</Th><Th>R$/frasco</Th><Th>Honorário total</Th><Th>Data p/ honorário</Th><Th>Situação</Th><Th>Status</Th><Th>Nota fiscal</Th></tr></thead><tbody>{groups.map((entries) => {
      const sale = entries[0].sale;
      const paid = entries.filter((item) => item.commissionRecord).length;
      const pending = entries.find((item) => !item.commissionRecord);
      const total = entries.reduce((sum, item) => sum + item.commissionValue, 0);
      const latestPayment = [...entries].reverse().find((item) => item.payment)?.payment;
      const invoice = entries.find((item) => item.invoice)?.invoice;
      const paymentStatus = pending?.status ?? "Recebida";
      return <Fragment key={sale.id}><tr><Td strong>{sale.patientName}</Td><Td>{sale.doctor}</Td><Td>{sale.treatment}</Td><Td>{formatMoney(sale.contractedValue)}</Td><Td><button type="button" onClick={() => toggle(sale.id)} aria-expanded={isOpen(sale.id)} className="inline-flex items-center gap-2 rounded-lg border border-[#e5d9d4] bg-white px-3 py-2 text-xs font-bold text-[#a3113a]">☰ {isOpen(sale.id) ? "Ocultar parcelas" : "Ver parcelas"}</button></Td><Td strong>{formatMoney(sale.contractedValue)}</Td><Td>{pending ? formatDate(pending.dueAt) : "Quitado"}</Td><Td>{formatDate(latestPayment?.paidAt)}</Td><Td>{latestPayment?.method ?? sale.paymentMethod}</Td><Td>{entries[0].bottleCount}</Td><Td>{formatMoney(entries[0].commissionPerBottle)}</Td><Td strong>{formatMoney(total)}</Td><Td>{pending ? formatDate(pending.commissionAt) : "Quitado"}</Td><Td><span className={`rounded-full px-2 py-1 text-[10px] font-bold ${paid === entries.length ? "bg-[#edf8f3] text-[#187157]" : "bg-[#fff4e4] text-[#966419]"}`}>{paid}/{entries.length} pago(s)</span></Td><Td>{paymentStatus}</Td><Td>{invoice ? <button type="button" onClick={() => openInvoice(invoice)} className="font-bold text-[#a3113a]">Abrir</button> : "Não vinculada"}</Td></tr>{isOpen(sale.id) && <tr><td colSpan={16} className="border-b border-[#eee7e3] bg-[#fcfaf8] p-4"><div className="overflow-x-auto"><table className="w-full min-w-[1100px] text-left text-xs"><thead><tr><Th>Parcela</Th><Th>Valor</Th><Th>Vencimento</Th><Th>Pagamento</Th><Th>Forma</Th><Th>Frascos</Th><Th>R$/frasco</Th><Th>Honorário</Th><Th>Data p/ honorário</Th><Th>Situação</Th><Th>Ação</Th></tr></thead><tbody>{entries.map((item) => <tr key={item.id}><Td>{item.number}/{item.sale.installments}</Td><Td>{formatMoney(item.scheduledValue)}</Td><Td>{formatDate(item.dueAt)}</Td><Td>{formatDate(item.payment?.paidAt)}</Td><Td>{item.payment?.method ?? item.sale.paymentMethod}</Td><Td>{item.bottleCount}</Td><Td>{formatMoney(item.commissionPerBottle)}</Td><Td strong>{formatMoney(item.commissionValue)}</Td><Td>{formatDate(item.commissionAt)}</Td><Td>{item.commissionRecord ? <span className="rounded-full bg-[#edf8f3] px-2 py-1 font-bold text-[#187157]">Pago</span> : item.commissionAt && item.commissionAt <= today ? <span className="rounded-full bg-[#fff4e4] px-2 py-1 font-bold text-[#966419]">Disponível</span> : <span className="rounded-full bg-[#eef3ff] px-2 py-1 font-bold text-[#3c5da0]">Agendado</span>}</Td><Td>{commissionAction(item)}</Td></tr>)}</tbody></table></div></td></tr>}</Fragment>;
    })}</tbody></table>{!groups.length && <p className="py-10 text-center text-sm text-[#817578]">Nenhum honorário encontrado.</p>}</div>;
  }

  return <div className="mt-4 max-h-[680px] overflow-auto"><table className="w-full min-w-[1000px] text-left text-sm"><thead className="sticky top-0 bg-white"><tr><Th>Paciente</Th><Th>Médico</Th><Th>Tratamento</Th><Th>Venda</Th><Th>Parcela</Th><Th>Valor</Th><Th>Vencimento</Th><Th>Pagamento</Th><Th>Forma</Th><Th>Status</Th><Th>Nota fiscal</Th></tr></thead><tbody>{installments.map((item) => <tr key={item.id}><Td strong>{item.sale.patientName}</Td><Td>{item.sale.doctor}</Td><Td>{item.sale.treatment}</Td><Td>{formatMoney(item.sale.contractedValue)}</Td><Td>{item.number}/{item.sale.installments}</Td><Td strong>{formatMoney(item.scheduledValue)}</Td><Td>{formatDate(item.dueAt)}</Td><Td>{formatDate(item.payment?.paidAt)}</Td><Td>{item.payment?.method ?? item.sale.paymentMethod}</Td><Td>{statusPill(item)}</Td><Td>{item.invoice ? <button type="button" onClick={() => openInvoice(item.invoice)} className="font-bold text-[#a3113a]">Abrir</button> : "Não vinculada"}</Td></tr>)}</tbody></table>{installments.length === 0 && <p className="py-10 text-center text-sm text-[#817578]">Nenhuma parcela encontrada.</p>}</div>;
}
