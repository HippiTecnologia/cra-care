...
export default function NursingPage() {
  ...
  const [doctors, setDoctors] = useState<NursingDoctor[]>([]);
  ...
  async function reload() {
    ...
    setDoctorId((current) => current || workspace.doctors[0]?.id || "");
  ...
  async function savePatient(event: FormEvent<HTMLFormElement>) {
    ...
    if (!profile) return;
    if (!doctorId) { setMessage("Selecione o médico responsável pela paciente."); return; }
    const responsibleDoctor = doctors.find((doctor) => doctor.id === doctorId);
    const id = await createNursingPatient(profile, { name, cpf, birthDate, phone, doctorId });
    const patient: NursingPatient = { id, name, cpf, birthDate, phone, doctorId, doctorName: responsibleDoctor?.fullName ?? "Médico responsável", createdAt: new Date().toISOString() };
