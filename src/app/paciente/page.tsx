...
function openPrintableDocument(title: string, sections: { heading: string; text: string }[]) {
  ...
  popup.document.write(`<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><title>${escapeHtml(title)}</title><style>body{font-family:Arial,sans-serif;color:#34292d;max-width:760px;margin:48px auto;padding:0 24px}header{display:flex;align-items:center;justify-content:space-between;gap:20px;border-bottom:2px solid #a3113a;padding-bottom:20px}header img{width:74px;height:74px;border-radius:14px}h1{color:#a3113a;font-size:26px}h2{font-size:16px;color:#86203b;margin-top:28px}p{font-size:14px;line-height:1.8}footer{margin-top:48px;border-top:1px solid #eadfd9;padding-top:16px;color:#776b6e;font-size:12px}.document-actions{position:fixed;right:22px;top:22px;display:flex;gap:10px;z-index:2}.document-actions button{border:0;border-radius:10px;padding:11px 14px;font:600 13px Arial,sans-serif;cursor:pointer}.print{background:#a3113a;color:white}.close{background:white;color:#a3113a;border:1px solid #d9c9cc!important}@media print{body{margin:20px auto}.document-actions{display:none}}</style></head><body><div class="document-actions"><button class="print" onclick="window.print()">Imprimir / salvar em PDF</button><button class="close" onclick="window.close()">Fechar e voltar</button></div><header><div><strong>CRA Care · Centro de Rinite e Alergia</strong><h1>${escapeHtml(title)}</h1></div><img src="${window.location.origin}/icon.png" alt="CRA Care"></header>${content}<footer>Documento gerado pelo portal CRA Care · ${escapeHtml(formatDate(new Date().toISOString(), true))}</footer></body></html>`);
