...
function openPrintableDocument(title: string, sections: { heading: string; text: string }[]) {
  ...
  popup.document.write(`<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><title>${escapeHtml(title)}</title><style>@page{size:A4 portrait;margin:14mm}*{box-sizing:border-box}body{margin:0;background:#f6f1ef;font-family:Arial,sans-serif;color:#34292d}.document-shell{max-width:820px;margin:0 auto;padding:28px 20px 56px}.document-card{border-radius:24px;background:#fff;padding:42px;box-shadow:0 18px 50px rgba(52,41,45,.14)}header{display:flex;align-items:center;justify-content:space-between;gap:20px;border-bottom:2px solid #a3113a;padding-bottom:16px}header img{width:70px;height:70px;object-fit:contain}h1{margin:7px 0 0;color:#a3113a;font-size:25px}h2{font-size:16px;color:#86203b;margin:26px 0 9px}p{font-size:13px;line-height:1.7}section{break-inside:avoid}footer{margin-top:38px;border-top:1px solid #eadfd9;padding-top:14px;color:#776b6e;font-size:11px}.document-actions{display:flex;flex-wrap:wrap;justify-content:flex-end;gap:10px;margin-bottom:16px}.document-actions button{border:0;border-radius:10px;padding:11px 14px;font:600 13px Arial,sans-serif;cursor:pointer}.print{background:#a3113a;color:white}.close{background:white;color:#a3113a;border:1px solid #d9c9cc!important}@media print{body{background:#fff}.document-shell{max-width:none;padding:0}.document-card{border-radius:0;padding:0;box-shadow:none}.document-actions{display:none}}</style></head><body><main class="document-shell"><div class="document-actions"><button class="print" onclick="window.print()">Imprimir / salvar em PDF</button><button class="close" onclick="window.close()">Fechar e voltar</button></div><article class="document-card"><header><div><strong>CRA CARE</strong><small style="display:block;margin-top:3px;color:#66595d;font-size:10px;letter-spacing:.5px">CENTRO DE RINITE E ALERGIA</small><h1>${escapeHtml(title)}</h1></div><img src="${window.location.origin}/logo-cra.png" alt="CRA Care"></header>${content}<footer>Documento gerado pelo portal CRA Care · ${escapeHtml(formatDate(new Date().toISOString(), true))}</footer></article></main></body></html>`);
...
export default function PatientPortalPage() {
  ...
  function startBottle() {
    ...
    if (!latestPrescription || prescribedBottleCount < 1) {
      setMessage("Aguarde a receita do médico antes de iniciar um frasco.");
      return;
    }
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-[#faf7f5] pb-24 text-[#34292d] lg:pb-0">
          ...
          <section className="relative z-10 mx-auto mt-0 max-w-5xl px-4 pb-8 pt-5 sm:px-6 lg:px-10">
            ...
            {section === "alertas" && (
              <article className="rounded-[28px] border border-[#eee5e0] bg-white p-5 shadow-sm sm:p-7">
                ...
                <label className="mt-7 block min-w-0 text-sm font-bold text-[#433438]">Horário do lembrete<span className="mt-2 block w-full max-w-full overflow-hidden rounded-xl border border-[#e9dfda] bg-white sm:max-w-48"><input type="time" value={reminderDraft.time} onChange={(event) => setReminderDraft((current) => ({ ...current, time: event.target.value }))} className="block h-12 w-full min-w-0 max-w-full border-0 bg-transparent px-4 font-normal outline-none" /></span></label>
                ...
                <div className="mt-6 rounded-2xl bg-[#fff7ea] p-4 text-xs leading-6 text-[#806238]"><strong>Avisos no celular:</strong> ao salvar os alertas, permita as notificações do CRA Care. A Secretaria poderá enviar avisos mesmo com o portal fechado.{permission === "granted" && <p className="mt-2 font-semibold text-[#187157]">Notificações permitidas neste dispositivo.</p>}{permission === "denied" && <p className="mt-2 font-semibold text-[#a3113a]">As notificações estão bloqueadas neste navegador. Abra as permissões do site e permita notificações.</p>}{permission === "unsupported" && <p className="mt-2 font-semibold text-[#a3113a]">Este navegador não permite notificações. Abra pelo navegador normal ou pelo ícone instalado no celular.</p>}</div>
              </article>
            )}
            ...
            {section === "notas" && (
              <article className="rounded-[28px] border border-[#eee5e0] bg-white p-5 shadow-sm sm:p-7">
                ...
                {patientNotes.filter((note) => note.id !== "immunotherapy-guidance" && note.id !== `${latestPrescription?.id}-posology`).map((note) => <article key={note.id} className="mt-4 rounded-2xl border border-[#eee6e2] bg-[#fdfbf9] p-5"><p className="text-sm font-bold text-[#a3113a]">{note.title}</p><p className="mt-3 whitespace-pre-line text-sm leading-7 text-[#65585c]">{note.text}</p></article>)}
              </article>
            )}
      ...
      <a
        ...
        className="fixed bottom-36 right-4 z-40 flex h-14 w-14 items-center justify-center rounded-full bg-[#25D366] text-white shadow-[0_10px_30px_rgba(37,211,102,0.38)] transition hover:scale-105 hover:bg-[#1fbd5a] lg:bottom-6 lg:right-6 lg:h-auto lg:w-auto lg:gap-2 lg:px-5 lg:py-3"
      >
